...
func (cpuset *CpuSet) Set(cgroupPath string, resource *ResourceConfig) error {
	if cpusetCgroupPath, err := cgroup.GetCgroupPath(cpuset.Name(), cgroupPath, true); err == nil {
		if resource.CpuSet != "" {
...
func (cpuset *CpuSet) Remove(cgroupPath string) error {
	if cpuCgroupPath, err := cgroup.GetCgroupPath(cpuset.Name(), cgroupPath, false); err == nil {
